...
const MycroftServiceManager = new Lang.Class({
  ...
  stopService: function (callback) {
    ...
    try {
      ...
      log(e);
        ...
        // this.myUi.topMenuBar.emit('mycroft-status-disabled');
    if (callback) {
      callback();
    }
    this.emitServiceStatus('disabled');
  ...
  getServiceStatus: function (callback) {
    ...
    if (mycroft_is_install) {
      ...
      callback('install');
        ...
        // try {
        //     let [res, out] = GLib.spawn_command_line_sync('ps -ef | grep mycroft');
        //     print(out);
        //     // let [res, out] = GLib.spawn_command_line_sync('ps -ef | grep abc | grep -v grep');
        //     // log('Result: '+ res);
        //     // log('Out:' + out);
        //     // if(res.length>1){
        //     //     log(out);
        ...
        socketClient.websocket_connect_async(message, null, null, null, Lang.bind(this, function (session, result) {
          try {
            ...
            if (this.connection !== null) {
              this.connection.connect('message', Lang.bind(this, this.onMessage)); // function(con, type, message) {
                                // this.onMessage(con, type, message)
                                // }));
              this.connection.connect('closed', Lang.bind(this, function (con) {
                this.onClosed(con);
              }));
                            // this.connection.connect('closing', Lang.bind(this, function(con) {
                            //     this.onClosing(con);
                            // }));
                            // this.connection.connect('error', Lang.bind(this, function(con, error) {
                            //     this.onError(con, error);
                            // }));

              this.wsStarted = true;
            }
          } catch (e) {
                        // this.myUi.topMenuBar.emit('disabled');
            this.emitServiceStatus('failed'); // Failed();
                        // this.myUi.topMenuBar.emit('mycroft-status-failed');
          }
        }));
      } catch (e) {
        log(e);
      }
            // socketClient.websocket_connect_async(message, null, null, null, Lang.bind(this, function(session, res) {
  ...
  emitServiceStatus: function (status, arg) {
    if (status === 'starting' || status === 'stopping') {
      ...
      this.locked = false;
        ...
        // this.myUi.displayBox.searchBox.topMenuBar.emit('mycroft-status', status);
  ...
  destroy: function () {
    ...
    // emitServiceStarting: function() {
    //     //this.myUi.topMenuBar.disconnect(this.serviceClicked);
    //     //log('emitStart');
    //     this.locked = true;
    //     this.myUi.topMenuBar.emit('mycroft-status-starting');
    // },
    // emitServiceActive: function() {
    //     this.locked = false;
    //     this.myUi.topMenuBar.emit('mycroft-status-active');
    // },
    // emitServiceFailed: function() {
    //     this.myUi.topMenuBar.emit('mycroft-status-failed');
    //     this.locked = false;
    // },
    // emitServiceDisabled: function() {
    //     log('emitDisabled');
    //     this.myUi.topMenuBar.emit('mycroft-status-disabled');
    //     this.locked = false;
    // },
    // emitServiceStopping: function() {
    //     log('emitStopping');
    //     this.myUi.topMenuBar.emit('mycroft-status-stopping');
    //     this.locked = true;
    // },
    // emitServiceNotInstalled: function() {
    //     this.locked = true;
    //     this.myUi.topMenuBar.emit('mycroft-status-not-installed');
    // },
    // emitServiceListening: function(val) {
    //     if (val === 'start') {
    //         this.myUi.topMenuBar.emit('mycroft-status-listening','start');
    //     } else if(val === 'stop') {
    //         this.myUi.topMenuBar.emit('mycroft-status-listening','stop');
    //     }
    // },
    // get _mycroft_is_install_key() {
...
const MycroftUI = new Lang.Class({
  ...
  _init: function () {
    ...
    if (Me.metadata.version !== undefined && Me.metadata.version.toString().trim() !== '') {
      ...
      this.user_agent += Me.metadata.version.toString();
        ...
        // add trailing space, so libsoup adds its own user-agen
    this.user_agent += ' ';
    ...
    this.setEventListeners();
        ...
        // this.myUi.core_location = this._core_location;
    ...
    Main.panel.addToStatusArea('mycroftAi', this.mycroftPanel);
        ...
        // let mycroftService = new MycroftServiceManager();
    applyStyles();
  },
  setEventListeners: function () {
    ...
    this.mycroftServiceSettingsChangedId = this.mycroftService.connect('settings-changed', Lang.bind(this, function (uploader, status) {
      this.mycroftPanel.checkPositionInPanel();
            // this.myUi.displayBox.searchBox.barAnimation.stopAnimation(uploader, status);
    }));
        // this.mycroftPanelId = this.mycroftPanel.connect('user-installed', Lang.bind(this, function(uploader, status) {
        //     this.updateStatus(uploader, status);
        //     this.myUi.displayBox.searchBox.barAnimation.stopAnimation(uploader, status);
        // }));
    this.mycroftServiceStatusId = this.mycroftService.connect('mycroft-status', Lang.bind(this, this.updateStatus));
        // this.mycroftService.connect('mycroft-status', Lang.bind(this.displayBox.searchBox, this.updateStatusInfo));
        // this.mycroftService.connect('mycroft-status', Lang.bind(this.mycroftPanel, this.mycroftPanel.updatePanelIcon));
  ...
  destroy: function () {
    this.destroySignals();
    if (this.mycroftServiceId) {
      this.mycroftService.disconnect(this.mycroftPanelId);
      this.mycroftPanelId = 0;
    }
    if (this.mycroftServiceId) {
      this.mycroftService.disconnect(this.mycroftPanelId);
      this.mycroftPanelId = 0;
...
const MycroftPanelButton = new Lang.Class({
  ...
  _init: function () {
    this._position_in_panel = position_in_panel;
        ...
        // let screen_image = Me.dir.get_child('icons').get_child("mycroftLogo.svg");
        // // If the theme-independent image is there, use it...

        // this._mycroftIcon = new Clutter.Texture({
        //     filter_quality: Clutter.TextureQuality.HIGH,
        //     //effect: this.colorizeEffect
        // });

        // this._mycroftIcon.set_from_file(screen_image.get_path())
    ...
    //do nothing
        ...
        // core_location = this.core_location;
        // mycroft_is_install = this.mycroft_is_install;
        // Position icon in Top Panel Box
  ...
  updatePanelIcon: function (status) {
    ...
    //         if (mycroft_is_install !== mycroft_is_install_change) {
    //             mycroft_is_install = this.mycroft_is_install;
    //             this.emit('user-installed', 'disabled');
    //         } else if (mycroft_is_install_change === false) {
    //             this.emit('user-installed', 'install');
    //         }
    //     }));
    // },
    // get mycroft_is_install() {
...
const MycroftPopup = new Lang.Class({
  ...
  _init: function () {
    this.topMenuBar = new TopMenuBar();
        // Putting the popup (Main Window) item together
    ...
    this.popupMenuMain = new PopupMenu.PopupBaseMenuItem({
      ...
      can_focus: false,
        ...
        // this._menuBar = new SliderItem("document-open-recent-symbolic", 0);
    ...
    this.mainBox.add_actor(this.displayBox.actor);
        // return this._itemMycroftPopup;
        // printAllProperties(this._menuBar);
  },
  ...
  updateStatus: function (status) {
    ...
    case 'starting':
      ...
      this.label.set_text('Your Mycroft Assistant is just starting up. Please wait');
                // this.label.add_style_class_name('starting');
      this.sBox.add_actor(this.label);
      break;
    case 'disabled':
    ...
    //do nothing
        ...
        //     this.actor.add_actor(this.conversationBox.actor);
        //     this.actor.add_actor(this.chatBox.messageBox);
...
const HintBox = new Lang.Class({
  ...
  _init: function () {
    this.actor = new St.BoxLayout({
      ...
      x_expand: true,
      y_expand: true,
            //x_align:Clutter.ActorAlign.CENTER,x_fill:true
    });
...
const ConversationBox = new Lang.Class({
  ...
  _init: function () {
    ...
    this.conversationView = new St.BoxLayout({
      ...
      vertical: true,
        ...
        // this.chatBox._suggestionsResults._suggestionsRandom();
  ...
  addMessage: function (uploader, message, reciever) {
    ...
    this.conversationView.add_actor(label);
        ...
        //     this.conversationView.remove_actor(this._initMessage);
        // }
    if (this.conversationView.get_n_children() > 1) {
      if (this.clearId === 0) {
...
const ChatBox = new Lang.Class({
  ...
  _init: function () {
    ...
    this._clearIcon = new St.Icon({
      ...
      icon_name: 'edit-clear-symbolic',
        ...
        // primary-icon-clicked does not throw event. REPORT this to GNOME.
    ...
    this._primaryIconClickedId = this._primaryIcon.connect('button-release-event', Lang.bind(this, this._onPrimaryIconClick));
        //      log(this._primaryIconClickedId);


    this._secondaryIconClickedId = 0;
        // this._suggestionsPage = new St.Bin({ child: this._suggestionsResults.actor,
        //                         x_align: St.Align.START,
        //                         y_align: St.Align.START,
        //                         x_fill: true,
        //                         y_fill: true });
        // this._suggestionsPage.hide();
        // this.actor.add_actor(this.messageBox);
    this.messageBox.add_actor(this._entryBin);
        // this.messageBox.add_actor(this._suggestionsPage);
    this.setEventListeners();
        // this._isMuted();
  },
  setEventListeners: function () {
  ...
  _onTextChanged: function (se, prop) {
    ...
    if (this._suggestionsActive) {
            // this._suggestionsResults._showSuggestions(this._entry.get_text());
  ...
  _onPrimaryIconClick: function () {
        ...
        // this._primaryIcon.icon_name = 'microphone-sensitivity-muted-symbolic.svg';
  ...
  _isActivated: function () {
        // log(this._text.text == this._entry.get_text());
...
const SuggestionsBox = new Lang.Class({
  ...
  showSuggestFocus: function (uploader, event) {
    if (uploader.has_style_pseudo_class('focus') || uploader.has_style_pseudo_class('hover')) {
      ...
      this.suggestFocus.add_style_class_name('suggestions-focus-label-active');
            ...
            // this.suggestFocus.set_x(uploader.get_x());
            // this.suggestFocus.set_y(uploader.get_y()+2);
    } else {
  ...
  wordSuggest: function (keywordToSearch) {
    ...
    let len = 0,
        ...
        // let baseLocation = "/usr/share/plasma/plasmoids/org.kde.plasma.mycroftplasmoid/contents/ui/suggestion/";
        // let files = [{
        //     "id": "1",
        //     "category": "math",
        //     "keywordFile": "+,-,/,*,add,subract,multiply,divide,calculate,pie,square,squareroot",
        //     "listFile": "Calculate 2+2,Calculate 1/4 * (4 - 1/2),pi to 10 digits,219 to binary,Calculate e^z",
        // }, {
        //     "id": "2",
        //     "category": "general",
        //     "keywordFile": "time,joke,hello,world,invented,how,what,distance",
        //     "listFile": "tell me a joke,what is the current time,hello world,how are you,which star is at the center of our Solar System?,what is the diameter of Sun hello world hello hello world?,how many stars are in the Milky Way?",
        // }];
    // let baseSearch, keywords, keywordsFile, list, listFile, suggestion = [];
...
const MycroftBarAnimation = new Lang.Class({
  ...
  animateBox: function () {
    ...
    this.actor.add_effect_with_name('test1234', this.colorizeEffect);

    Tweener.removeTweens(this.colorizeEffect);
    Tweener.addTween(this.colorizeEffect, {
      time: 2,
      delay: 0.5,
      factor: 1,
      scale_x: 1,
    ...
    Tweener.addTween(this.colorizeEffet);
  },
...
const TopMenuBar = new Lang.Class({
  ...
  updateStatusLabelText: function (status) {
    if (status === 'failed') {
      ...
      this.statusLabel.set_text('Mycroft is not installed please install from the settings panel');
    } else {
  ...
  destroy: function () {
        ...
        //         this.disconnect(this.updateStatusId);
        //         this.updateStatusId = 0;
        // }
        // if (this.hintActorId) {
        //         this.hintActor.disconnect(this.hintActorId);
        //         this.hintActorId = 0;
        // }
        //  if (this.searchActorId) {
        //         this.searchActor.disconnect(this.searchActorId);
        //         this.searchActorId = 0;
        // }
...
// Remove the padding and change color of the popup
function getColor(status) {
  ...
  if (status === 'active') {
    ...
    g = 133,
    b = 67;
  } else if (status === 'failed' || status === 'install') {
    r = 179,
    g = 133,
    b = 67;
...
function findPanelClass(className) {
    // Too hacky. Need to find a better way.
  let pane = {};
